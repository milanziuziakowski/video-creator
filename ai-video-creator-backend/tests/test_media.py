...
async def test_upload_first_frame(
    ...
    with patch(
        "app.services.media_service.MediaService.validate_image", new_callable=AsyncMock
    ) as mock_validate:
        mock_validate.return_value = (True, None)

        with patch(
            "app.services.media_service.MediaService.save_upload", new_callable=AsyncMock
        ) as mock_save:
            ...
            with patch(
                "app.services.project_service.ProjectService.set_first_frame_url",
...
async def test_upload_audio_sample(
    ...
    with patch(
        "app.services.media_service.MediaService.validate_audio", new_callable=AsyncMock
    ) as mock_validate:
        mock_validate.return_value = (True, None)
        ...
        with patch(
            ...
            with patch(
                "app.services.project_service.ProjectService.set_audio_sample_url",
                new_callable=AsyncMock,
            ):
                with patch(
                    "app.services.project_service.ProjectService.update_project_status",
                    new_callable=AsyncMock,
                ):
                    response = await async_client.post(
